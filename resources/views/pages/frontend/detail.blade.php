                        ...
                        @foreach ($product->galleries as $gallery)
                            <div class="px-2">
                                <div class="item {{ $loop->first ? 'selected' : '' }}"
                                    data-img="{{ Storage::url($gallery->url) }}">
                                    <img src="{{ Storage::url($gallery->url) }}" alt="front"
        ...
        <div class="container mx-auto">
            <div class="flex flex-start mb-4">
                <h3 class="text-2xl capitalize font-semibold">
                    Complete your room <br class="___class_+?30___" />with what we designed
                </h3>
            </div>
                    ...
                    <div class="px-3 flex-none" style="width: 320px">
                        <div class="rounded-xl p-4 pb-8 relative bg-white">
                            <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                                <img src="{{ $r->galleries()->exists() ? Storage::url($r->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}"
                                    alt="" class="w-full h-full object-cover object-center" />
                            </div>
                            <h5 class="text-lg font-semibold mt-4">{{ $r->name }}</h5>
                            <span class="___class_+?37___">IDR {{ number_format($r->price) }}</span>
                            <a href="{{ route('detail', $r->slug) }}" class="stretched-link">
                                <!-- fake children -->
                            </a>
                        </div>
                    </div>
